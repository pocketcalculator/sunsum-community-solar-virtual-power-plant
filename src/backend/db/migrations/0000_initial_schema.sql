...
CREATE TABLE "assessments" (
	...
	CONSTRAINT "assessments_size_range_check" CHECK ("assessments"."estimated_system_size_kw_low" IS NULL OR "assessments"."estimated_system_size_kw_high" IS NULL OR "assessments"."estimated_system_size_kw_low" <= "assessments"."estimated_system_size_kw_high"),
	CONSTRAINT "assessments_generation_range_check" CHECK ("assessments"."estimated_annual_generation_kwh_low" IS NULL OR "assessments"."estimated_annual_generation_kwh_high" IS NULL OR "assessments"."estimated_annual_generation_kwh_low" <= "assessments"."estimated_annual_generation_kwh_high")
);
--> statement-breakpoint
...
CREATE TABLE "funding_needs" (
	...
	CONSTRAINT "funding_needs_amount_requested_check" CHECK ("funding_needs"."amount_requested" > 0),
...
CREATE TABLE "investor_engagements" (
	...
	"decline_reason" text,
	"created_at" timestamp with time zone DEFAULT now() NOT NULL,
	CONSTRAINT "investor_engagements_unique" UNIQUE NULLS NOT DISTINCT("investor_id","project_id","funding_need_id"),
...
CREATE TABLE "investors" (
	...
	CONSTRAINT "investors_ticket_size_check" CHECK ("investors"."ticket_size_min" IS NULL OR "investors"."ticket_size_max" IS NULL OR "investors"."ticket_size_min" <= "investors"."ticket_size_max"),
...
CREATE TABLE "sites" (
	...
	"created_at" timestamp with time zone DEFAULT now() NOT NULL,
	"updated_at" timestamp with time zone DEFAULT now() NOT NULL,
	CONSTRAINT "sites_site_type_check" CHECK ("sites"."site_type" IN ('rooftop', 'land')),
	CONSTRAINT "sites_ownership_status_check" CHECK ("sites"."ownership_status" IN ('confirmed', 'pending', 'unverified')),
	CONSTRAINT "sites_submission_status_check" CHECK ("sites"."submission_status" IN ('draft', 'submitted', 'in_review', 'info_requested', 'accepted', 'rejected')),
...
ALTER TABLE "diligence_requests" ADD CONSTRAINT "diligence_requests_engagement_id_investor_engagements_id_fk" FOREIGN KEY ("engagement_id") REFERENCES "public"."investor_engagements"("id") ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
...
ALTER TABLE "investor_engagements" ADD CONSTRAINT "investor_engagements_funding_need_id_funding_needs_id_fk" FOREIGN KEY ("funding_need_id") REFERENCES "public"."funding_needs"("id") ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
