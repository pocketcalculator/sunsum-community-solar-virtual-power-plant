 ...
 *
 * `SUNSUM_STORE=db` reads PostgreSQL; unset or `mock` uses the in-memory
 * fixtures. Any other value is rejected. The default is the mock so that
 * `npm run dev`, `npm test` and CI work with no database and no `DATABASE_URL`
 * at all.
 *
