...
describe("the bounded Azure preparation contract", () => {
  it("reuses the canonical schema and migrations with a separate explicit Azure operator command", () => {
    ...
    expect(read("scripts/db-migrate.ts")).toContain(
      "../src/backend/db/migrations",
    );
    expect(read("src/backend/infrastructure/database/pool.ts")).toContain(
      'from "../../db/schema"',
    );
    expect(read("src/backend/infrastructure/database/pool.ts")).not.toContain(
      'from "../../db"',
    );
    expect(
      existsSync(join(root, "src/backend/infrastructure/database/schema.ts")),
    ).toBe(false);
  ...
  it("creates the complete dev test stack without existing resource references", () => {
    ...
    expect(core).not.toMatch(
      /webAppMode|postgresMode|existingPostgres|\bexisting\s*=/u,
    );
  ...
  it("requires approved identities and code-flow credentials in a separate sign-in template", () => {
    ...
    expect(auth).toMatch(
      /@minLength\(1\)\s+@maxLength\(13\)\s+param approvedParticipantObjectIds string\[\]/u,
    );
  ...
  it("keeps provisioning SKU guards separate from plan-independent code deployment", () => {
    ...
    const validation = read(
      "infrastructure/scripts/InfrastructureValidation.psm1",
    );
    ...
    expect(deploy).toContain(
      "Get-ValidatedInfrastructureChanges -Result $result",
    );
    ...
    expect(
      read("infrastructure/scripts/Deploy-AppServiceCode.ps1"),
    ).not.toContain("Assert-AppServiceFreePlan");
    ...
    expect(web).not.toMatch(
      /name: 'PORT'|WEBSITE_RUN_FROM_PACKAGE|DOCKER\||containerapp|appinsights/i,
    );
    ...
    expect(core).not.toMatch(
      /resourceGroups@|ContainerRegistry|containerApps/u,
    );
    const provision = read(
      "infrastructure/scripts/Provision-Infrastructure.ps1",
    );
  ...
  it("skips deployment safely until reviewed deployment artifacts are configured", () => {
    const workflow = read(".github/workflows/deploy-azure.yml");
    expect(workflow).toContain("id: deployment-artifacts");
    expect(workflow).toContain('echo "configured=false" >> "$GITHUB_OUTPUT"');
    expect(workflow).toContain(
      "skipping Azure deployment until an operator provisions them.",
    );
    for (const step of [
      "Materialize reviewed parameter and approval records",
      "Azure Login",
      "Provision Infrastructure",
    ]) {
      expect(workflow).toMatch(
        new RegExp(
          `name: ${step}\\n\\s+if: steps\\.deployment-artifacts\\.outputs\\.configured == 'true'`,
          "u",
        ),
      );
    }
  ...
  it("declares a fixture-only web host with database configuration kept in operator outputs", () => {
    ...
    expect(web).not.toMatch(
      /PGPASSWORD|DATABASE_URL|SUNSUM_DB_AUTH|AZURE_CLIENT_ID/u,
    );
  });
  ...
  it("keeps network approval separate from Entra-only PostgreSQL provisioning", () => {
    ...
    expect(postgres).not.toMatch(
      /administratorLogin|firewallRules|delegatedSubnet/u,
    );
  ...
  it("shares one dev target across infrastructure and code deployment", () => {
    ...
    expect(existsSync(join(root, "infrastructure/config/code.dev.json"))).toBe(
      false,
    );
  ...
  it("compiles versioned dev sources and supports repeat deployments without Azure calls", () => {
    const config = JSON.parse(read("infrastructure/config/dev.json"));
    expect(config.infrastructure.templatePath).toBe(
      "../templates/resources.bicep",
    );
    expect(config.infrastructure.parametersPath).toBe(
      "../templates/resources.dev.bicepparam",
    );
    expect(config.infrastructure.deploymentName).toBe(
      "sunsum-dev-test-infrastructure",
    );
    ...
    const parameters = read(
      "infrastructure/templates/resources.dev.bicepparam",
    );
    ...
    expect(parameters).not.toMatch(
      /readEnvironmentVariable|stsunsumdev928e5e28|db-sunsum-dev-centralus/u,
    );
    expect(
      existsSync(
        join(root, "infrastructure/scripts/Deploy-DevInfrastructure.ps1"),
      ),
    ).toBe(false);
    const result = execFileSync(
      "pwsh",
  ...
  it.each(["firewall-template", "deployment-safety", "mvp-access"])(
    "propagates real %s failures through the CI shell wrapper",
    (suite) => {
  ...
  it("runs the actual bootstrap safety tests without database or identity calls", () => {
    const result = execFileSync(
      ...
      [
        ...
        join(
          root,
          "infrastructure/scripts/tests/postgres-bootstrap-operations.test.mjs",
        ),
      ],
  ...
  it("checks actual App Service HTTP responses without following redirects", () => {
    const result = execFileSync(
      ...
      [
        ...
        join(
          root,
          "infrastructure/scripts/tests/app-service-response.test.mjs",
        ),
      ],
