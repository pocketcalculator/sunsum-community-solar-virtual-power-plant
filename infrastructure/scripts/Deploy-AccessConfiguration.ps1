...
$inputSnapshot = New-DeploymentSnapshot -Path $ConfigurationPath -ExpectedSha256 $ExpectedSha256
$parameterSnapshot = $null
try {
    $path = $inputSnapshot.Path
    ...
    $parameterBytes = [System.Text.UTF8Encoding]::new($false).GetBytes($parameterContent)
    $parameterHash = [System.Convert]::ToHexString([System.Security.Cryptography.SHA256]::HashData($parameterBytes))
    Write-NewAccessRecord $output $parameterContent
    $parameterSnapshot = New-DeploymentSnapshot -Path $output -ExpectedSha256 $parameterHash
    if (-not $Apply) {
    ...
    if ($Operation -eq 'StorageNetwork') {
        ...
        Assert-DeploymentSnapshot $inputSnapshot
        Assert-DeploymentSnapshot $parameterSnapshot
    ...
    & az deployment group create --subscription $SubscriptionId --resource-group $ResourceGroupName `
        ...
        --parameters "@$($parameterSnapshot.Path)" --only-show-errors --output none
    ...
    try {
        if ($null -ne $parameterSnapshot) { Remove-DeploymentSnapshot $parameterSnapshot }
    } finally { Remove-DeploymentSnapshot $inputSnapshot }
}
